define([
  ...
  function sendSampleRequest(group, name, version, type)
  {
      ...
      for (var key in param) {
          ...
          if (url.match(regex)) {
            url = url.replace(regex, encodeURIComponent(param[key]));
            delete param[key];
          }
      }
      ...
      var regex = /\[?\?(.+)\]?/;
      var urlparams = url.match(regex);
      if (urlparams) {
        url = url.replace(regex, "");
        urlparams = urlparams[1].split("&");
        var queries = [];
        for (var urlparam of urlparams) {
            var is_empty = !urlparam.match(/=/);
            if (param[urlparam]) {
                if (is_empty) {
                    queries.push(urlparam);
                } else {
                    queries.push(urlparam + "=" + encodeURIComponent(param[urlparam]));
                }
