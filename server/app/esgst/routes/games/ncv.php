 ...
 */
$app->get('/games/ncv', function ($request, $response) {
  try {
...
$app->post('/games/ncv', function ($request, $response) {
  try {
    $filters = $request->getQueryParams();
...
function get_ncv_result($filters) {
  try {
...
function get_ncv($filters) {
  ...
  foreach (['app', 'sub'] as $type) {
    ...
    $query = implode(' ', [
      ...
      'FROM games__'.$type.'_ncv AS g_tr',
