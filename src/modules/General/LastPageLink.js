	...
	lpl_getLastPage(context, main, discussion, user, userWon, group, groupUsers, groupWishlist) {
		...
		if (paginationNavigation) {
			...
			if (
				...
				element.classList.contains('is_selected') ||
				...
				if ((main && window.location.pathname.match(/\/giveaways\/won/)) || userWon) {
					lastPage = Math.ceil(
						parseInt(
							context
								.querySelector(`.featured__table__row__right a[href*="/giveaways/won"]`)
					...
					lastPage = Math.ceil(
						parseInt(
							context
								.getElementsByClassName('sidebar__navigation__item__count')[0]
								.textContent.replace(/,/g, '')
						) / 25
					);
				}
				...
				if ((main && window.location.pathname.match(/\/users/)) || groupUsers) {
					lastPage = Math.ceil(
						parseInt(
							context
								.getElementsByClassName('sidebar__navigation__item__count')[1]
								.textContent.replace(/,/g, '')
						) / 25
					);
			...
			if (first) {
				...
				if (second) {
					...
					if (third && !third.textContent.match(/Giveaway\sFilters/)) {
						lastPage = Math.ceil(
							parseInt(third.textContent.replace(/,/g, '')) /
								((main && this.esgst.itemsPerPage) ||
									parseInt(second.textContent.replace(/,/g, '')) -
										parseInt(first.textContent.replace(/,/g, '')) +
										1)
						);
					}
				}
