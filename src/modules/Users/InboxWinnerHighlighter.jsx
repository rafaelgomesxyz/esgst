...
class UsersInboxWinnerHighlighter extends Module {
	constructor() {
		...
		this.info = {
			description: () => (
				<ul>
					<li>
						Adds an icon (<i className="fa fa-trophy"></i>) next to the username of a giveaway
						comment made by the giveaway's winner(s) (in the{' '}
						<a href="https://www.steamgifts.com/messages">inbox</a> page).
					</li>
					<li>
						A winner is only highlighted if you sent the gift to them after this feature was
						enabled.
					</li>
				</ul>
			),
	...
	async iwh_getUsers(context, main, source, endless) {
		if (
			!Shared.esgst.winnersPath &&
			!Shared.common.isCurrentPath('Messages') &&
			(!context.getAttribute || !context.getAttribute('data-esgst-qiv'))
		)
			return;
		const [callback, query] = Shared.esgst.winnersPath
				? [
						this.iwh_setObserver,
						`${
							endless
								? `.esgst-es-page-${endless} .table__gift-not-sent, .esgst-es-page-${endless}.table__gift-not-sent`
								: '.table__gift-not-sent'
						}`,
				  ]
				: [
						this.iwh_highlightWinner,
						`${
							endless
								? `.esgst-es-page-${endless} .comments__entity, .esgst-es-page-${endless}.comments__entity`
								: '.comments__entity'
						}`,
				  ],
	...
	iwh_setObserver(Context, winners) {
		...
		Username = Context.closest('.table__row-inner-wrap')
			.getElementsByClassName('table__column__heading')[0]
			.querySelector(`a[href*="/user/"]`).textContent;
	...
	iwh_highlightWinner(Context, Winners) {
		...
		Match = Context.firstElementChild.firstElementChild
			.getAttribute('href')
			.match(/\/giveaway\/(.+?)\//);
		if (Match) {
			Key = Match[1];
						...
						createElements(Context, 'afterend', [
							{
								attributes: {
									...
									title: getFeatureTooltip(
										'iwh',
										'This is the winner or one of the winners of this giveaway'
									),
								},
								type: 'i',
							},
