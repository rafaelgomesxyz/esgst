...
class GiveawaysIsThereAnyDealInfo extends Module {
			...
			description: () => (
				<ul>
					<li>
						...
						<a href="https://www.steamgifts.com/giveaway/aeqw7/">giveaway</a> page that shows the
						best current deal for the game, the historical lowest price of the game and a list with
						all of the bundles that the game has been in. All of this information is retrieved from{' '}
						<a href="https://isthereanydeal.com">IsThereAnyDeal</a>.
					</li>
					<li>
						Results are cached for 24 hours, so if you access a giveaway for the same game again
						within that timeframe, the information will not change.
					</li>
				</ul>
			),
			id: 'itadi',
	...
	async itadi_getInfo(currentTime, giveaway) {
		...
		if (
			game &&
			game.itadi &&
			game.itadi.version === 2 &&
			currentTime - game.itadi.lastCheck < 86400000
		) {
			itadi = game.itadi;
		} else {
	...
	async itadi_loadInfo(giveaway, plain) {
		...
		for (const deal of deals) {
			const match = deal.firstElementChild.textContent
				.trim()
				.match(/(Current\sBest|Historical\sLow)/);
			if (!match) {
				continue;
			}
	...
	itadi_addInfo(itadi, plain) {
		...
		if (itadi.current) {
			items[1].children.push(
				this.itadi_itemTemplate(
					`${itadi.current.source} (${itadi.current.date})`,
					`${itadi.current.price} (${itadi.current.cut})`,
					`https://isthereanydeal.com/game/${plain}/info/`
				)
			);
		} else {
			items[1].children.push(
				this.itadi_noItemTemplate('There are no current deals for this game.')
			);
		}
		if (itadi.historical) {
			items[3].children.push(
				this.itadi_itemTemplate(
					`${itadi.historical.source} (${itadi.historical.date})`,
					`${itadi.historical.price} (${itadi.historical.cut})`,
					`https://isthereanydeal.com/game/${plain}/info/`
				)
			);
		} else {
			...
			for (const bundle of itadi.bundles) {
				items[5].children.push(
					this.itadi_itemTemplate(
						...
						`${bundle.price} (${bundle.expiry})`,
						`https://isthereanydeal.com/specials/#/filter:id/${bundle.id}`
					)
				);
			}
		} else {
