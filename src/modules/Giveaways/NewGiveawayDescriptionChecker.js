			...
			description: [
				[
					...
					[
						[
							'li',
							`When you click on "Review Giveaway" in the new giveaway page (also extends to the "Create Giveaway" button from [id=gts] and the "Add" button from [id=mgc]), this feature checks if there are possible Steam keys / Humble Bundle gift links in the description and warns you about it, in case you pasted them there by mistake.`,
						],
						[
							'li',
							`This feature replaces the native "Review Giveaway" button, so that ESGST can intercept the click.`,
						],
					],
				],
			],
	...
	check(value) {
		return new Promise(async (resolve) => {
			...
			if (value.match(/[\d\w]{5}(-[\d\w]{5}){2,}/)) {
				...
				message =
					'There appears to be a Humble Bundle gift link in the description of the giveaway.';
			}

			if (message) {
				message = `${message} Are you sure you want to continue?`;
				await Shared.common.createConfirmation(
					message,
					() => resolve(false),
					() => resolve(true)
				);
