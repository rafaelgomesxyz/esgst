...
class GiveawaysSteamActivationLinks extends Module {
	constructor() {
		...
		this.info = {
			description: () => (
				<ul>
					<li>
						Adds 2 optional icons (<i className="fa fa-steam"></i> for the Steam client and{' '}
						<i className="fa fa-globe"></i> for the browser) next to each key in the "Key" column of
						your <a href="https://www.steamgifts.com/giveaways/won">won</a> page that allow you to
						quickly activate a won game on Steam, either through the client or the browser.
					</li>
	...
	sal_addObservers(context, main, source, endless) {
		const elements = context.querySelectorAll(
			`${
				endless
					? `.esgst-es-page-${endless} .view_key_btn, .esgst-es-page-${endless}.view_key_btn`
					: '.view_key_btn'
			}`
		);
	...
	sal_addLinks(context, main, source, endless) {
		const elements = context.querySelectorAll(
			`${
				endless
					? `.esgst-es-page-${endless} [data-clipboard-text], .esgst-es-page-${endless}[data-clipboard-text]`
					: `[data-clipboard-text]`
			}`
		);
		for (const element of elements) {
