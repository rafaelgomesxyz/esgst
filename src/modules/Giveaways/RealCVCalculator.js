...
class GiveawaysRealCVCalculator extends Module {
			...
			description: [
				[
					...
					[
						[
							'li',
							`Adds a "Real CV" row containing how much real CV you should get for a giveaway to the table of the review giveaway page (the page where you can confirm the creation of a giveaway).`,
						],
					],
				],
			],
	...
	async init() {
		if (this.esgst.newGiveawayPath) {
			if (!(await permissions.contains([['steamStore']]))) {
				return;
			}
				...
				if (game) {
					...
					try {
						...
						if (
							savedUser &&
							savedUser.giveaways &&
							savedUser.giveaways.sent &&
							savedUser.giveaways.sent[type][id]
						) {
