...
class GiveawaysGiveawayWinningRatio extends Module {
	constructor() {
		...
		this.info = {
			description: () => (
				<ul>
					<li>
						Adds an element (<i className="fa fa-pie-chart"></i> [Ratio]: 1) below a giveaway's
						start time (in any page) that shows the ratio (number of entries per copy) of the
						giveaway.
					</li>
					<li>
						The ratio is calculated by rounding up the result of the following formula:
						number_of_entries / number_of_copies
					</li>
			...
			features: {
				gwr_e: {
					description: () => (
						<ul>
							<li>The formula changes to: (number_of_entries + 1) / number_of_copies</li>
							<li>
								For example, if a giveaway has 2 copies and 6 entries, the current ratio is 3:1, but
								after you enter it, it will have 7 entries, so the ratio will increase to 4:1.
							</li>
						</ul>
					),
					name:
						'Show what the ratio will be when you enter the giveaway instead of the current ratio.',
					sg: true,
				},
				gwr_a: {
					description: () => (
						<ul>
							<li>
								Uses an advanced formula ((number_of_entries / time_open_in_milliseconds *
								duration_in_milliseconds) / number_of_copies) to calculate the ratio based on how
								much time the giveaway has been open and the duration of the giveaway. This gives
								you an estimate of what the ratio will be when the giveaway ends.
							</li>
						</ul>
					),
					features: {
				...
				gwr_h: {
					...
					description: () => (
						<ul>
							<li>
								Changes the color of the giveaway's title to the same color as the ratio and adds a
	...
	init() {
		...
		this.esgst.endlessFeatures.push(
			this.esgst.modules.giveawaysGiveawayWinningChance.gwc_addHeading.bind(
				this.esgst.modules.giveawaysGiveawayWinningChance
			)
		);
	}
			...
			if (
				...
				((giveaway.inviteOnly &&
					((main &&
						...
						giveaway.id)) ||
					!giveaway.inviteOnly) &&
				!giveaway.innerWrap.getElementsByClassName('esgst-gwr')[0]
			) {
				let context = createElements(
					giveaway.panel,
					Settings.get('gv') &&
	...
	gwr_addRatio(context, giveaway) {
		let advancedColor,
			...
			i;
		entries =
			giveaway.entered || giveaway.ended || giveaway.created || !Settings.get('gwr_e')
				? giveaway.entries
				: giveaway.entries + 1;
		basicRatio = Math.ceil(entries / giveaway.copies);
		if (Settings.get('gwr_a') && !giveaway.ended && giveaway.startTime) {
			advancedRatio = Math.ceil(
				((entries / (Date.now() - giveaway.startTime)) * (giveaway.endTime - giveaway.startTime)) /
					giveaway.copies
			);
		}
		...
		if (Settings.get('gwr_h')) {
			...
			giveaway.headingName.style.color =
				Settings.get('gwr_a') && !Settings.get('gwr_a_b') ? advancedColor : basicColor;
			if (giveaway.image) {
				...
				giveaway.image.style.boxShadow = `${
					Settings.get('gwr_a') && !Settings.get('gwr_a_b') ? advancedColor : basicColor
				} 0px 0px 0px var(--esgst-gwr-highlight-width, 3px)  inset`;
			}
		}
