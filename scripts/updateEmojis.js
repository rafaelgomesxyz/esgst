...
async function updateEmojis() {
	...
	const matches = text.match(
		/<td\sclass='code'><a\shref='#.+?'\sname='.+?'>.+?<\/a><\/td>[\s\S]+?<td\sclass='name'>.+?<\/td>/g
	);
	for (const match of matches) {
		const subMatches = match.match(
			/<td\sclass='code'><a\shref='#.+?'\sname='.+?'>(.+?)<\/a><\/td>[\s\S]+?<td\sclass='name'>(.+?)<\/td>/
		);
		const codes = subMatches[1]
			.trim()
			.split(/\s+/)
...
function formatName(name) {
	return name.replace(
		/(^|\s)([a-z])/g,
		(match, group1, group2) => `${group1}${group2.toUpperCase()}`
	);
}
