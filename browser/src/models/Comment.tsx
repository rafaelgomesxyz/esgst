...
abstract class Comment implements IComment {
	...
	static parseAll(context: HTMLElement, generation: number = 0, parent: IComment = null): IComment[] {
		switch (Session.namespace) {
			case Namespaces.SG: {
...
class SgComment extends Comment {
	...
	build(context: HTMLElement, position: string): void {
		...
		const patron = this.author.data.isPatron && (
			<i data-ui-tooltip="{&quot;rows&quot;:[{&quot;icon&quot; : [{&quot;class&quot; : &quot;fa-star&quot;, &quot;color&quot; : &quot;#84cfda&quot;}], &quot;columns&quot;:[{&quot;name&quot; : &quot;Patron&quot;}]}]}" class="fa fa-star"></i>
		);
					...
					<div id={this.data.code} class="comment__summary">
						<div class="comment__author">
							...
							{this.data.isDeleted ? (
								...
								<div class={`comment__username${this.author.data.isOp ? ' comment__username--op' : ''}`}>
									<a href={this.author.data.url}>{this.author.data.username}</a>
								</div>
							)}
							{this.author.data.roleId && (
								<a href={`/roles/${this.author.data.roleId}`} class="comment__role-name">{`(${this.author.data.roleName})`}</a>
							)}
							{this.author.data.isPatron && (Session.isLoggedIn ? (
								...
								<div class="comment__description">
									<form>
										...
										<div class="align-button-container">
											<a href="" class="comment__submit-button js__comment-edit-save">Save Changes</a>
											<div class="comment__cancel-button js__comment-edit-cancel"><span>Cancel</span></div>
										</div>
									</form>
								</div>
							</div>
						...
						{this.data.isDeleted ? (
							<div class="comment__delete-state">
								<div class="comment__description markdown markdown--resize-body">
									<p>This comment was deleted <span data-timestamp={this.data.deletedTimestamp}>{Shared.common.getTimeSince(this.data.deletedTimestamp)}</span> ago.</p>
								</div>
							</div>
						) : (
							<div class="comment__display-state">
								<div class="comment__description markdown markdown--resize-body">{this.data.markdown ? DOM.parse(Shared.esgst.markdownParser.text(this.data.markdown)).body.children : ''}</div>
							</div>
						)}
						<div class="comment__actions">
							<div>
								<span data-timestamp={this.data.createdTimestamp}>{Shared.common.getTimeSince(this.data.createdTimestamp)}</span> ago
								{this.data.isEdited && (
									<span data-timestamp={this.data.editedTimestamp}>*</span>
								)}
							</div>
							{this.data.canReply && (
							...
							{!this.data.isOp && (
								<a rel="nofollow noopener" href={this.data.url} class="comment__actions__button">Permalink</a>
							)}
						</div>
...
class StComment extends Comment {
	...
	parseData(): void {
		...
		if (authorNodes.reputation) {
			authorData.positiveReputation = parseInt(authorNodes.positiveReputation.textContent.slice(1).replace(',', '')); // +...
			authorData.negativeReputation = parseInt(authorNodes.negativeReputation.textContent.slice(1).replace(',', '')); // -...
		}
	...
	build(context: HTMLElement, position: string): void {
			...
			<fragment>
				<div class="author">
					...
					{this.data.isDeleted ? (
						...
						<fragment>
							<a href={this.author.data.url} class="author_avatar" style={`background-image:url(${this.author.data.avatar});`}></a>
							<a href={this.author.data.url} class={`author_name${this.author.data.isOp ? ' is_op' : ''}`}>{this.author.data.username}</a>
							<a href={this.author.data.url} class="author_small">
								(<span class="is_positive">+{this.author.data.positiveReputation}</span>/<span class="is_negative">-{this.author.data.negativeReputation}</span>)
							</a>
						</fragment>
					)}
				</div>
				<div class={this.data.isReview ? 'review_body' : 'comment_body'}>
					{this.data.isDeleted ? (
						<div class="comment_body_delete markdown">
							<p>This comment was deleted <span data-timestamp={this.data.deletedTimestamp}>{`${Shared.common.getTimeSince(this.data.deletedTimestamp)} ago`}</span>.</p>
						</div>
					) : (
						<div class={`${this.data.isReview ? 'review_description' : 'comment_body_default'} markdown`}>{this.data.markdown ? DOM.parse(Shared.esgst.markdownParser.text(this.data.markdown)).body.children : ''}</div>
					)}
					<div class="action_list">
						{this.data.isReview ? (
							<span>
								<span data-timestamp={this.data.createdTimestamp}>{`${Shared.common.getTimeSince(this.data.createdTimestamp)} ago`}</span>
							</span>
						) : (
							<div>
								<span data-timestamp={this.data.createdTimestamp}>{`${Shared.common.getTimeSince(this.data.createdTimestamp)} ago`}</span>
						...
						{this.data.canDelete && (
							<a data-form={`xsrf_token=${Session.xsrfToken}&do=comment_delete&comment_id=${this.data.id}`} class="js_comment_delete">Delete</a>
						)}
						{this.data.canUndelete && (
							<a data-form={`xsrf_token=${Session.xsrfToken}&do=comment_undelete&comment_id=${this.data.id}`} class="js_comment_undelete">Undelete</a>
						)}
						{this.data.sourceThreadTitle && (
							<div>
								{this.data.sourceCommentAuthor && (
									<fragment>
										<a rel="nofollow" href={this.data.sourceCommentUrl}>@{this.data.sourceCommentAuthor}</a>
						...
						{!this.data.isOp && (
							<a rel="nofollow" href={this.data.url}>Permalink</a>
						)}
					</div>
		...
		const outer = DOM.insert(context, position, (
			<div id={this.data.code} data-id={this.data.id} class={this.data.isReview ? 'review_outer' : 'comment_outer'}>
				{this.data.canEdit && (					
				...
				<div class={this.data.isReview ? 'review_inner' : 'comment_inner'} data-username={this.author.data.username}>
					{this.data.isReview ? (
						<fragment>
