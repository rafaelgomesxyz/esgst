...
class GiveawaysGiveawayPointsToWin extends Module {
  ...
  init() {
    if (!this.esgst.enteredPath && (!this.esgst.wonPath || !Settings.cewgd || !Settings.cewgd_w || !Settings.cewgd_w_e)) return;
  ...
  gptw_addPoints(giveaways, main, source) {
    for (const giveaway of giveaways) {
      if (giveaway.sgTools || (main && (this.esgst.createdPath || (this.esgst.wonPath && (!Settings.cewgd || !Settings.cewgd_w || !Settings.cewgd_w_e)) || this.esgst.newGiveawayPath || this.esgst.archivePath))) {
        continue;
      }
      if (((!giveaway.inviteOnly || ((!main || (!this.esgst.giveawayPath && !this.esgst.enteredPath && (!this.esgst.wonPath || !Settings.cewgd || !Settings.cewgd_w || !Settings.cewgd_w_e))) && main && !giveaway.ended && !giveaway.id)) && giveaway.inviteOnly) || giveaway.innerWrap.getElementsByClassName('esgst-gptw')[0]) {
        continue;
      }
      if (giveaway.started) {
        giveaway.gptwContext = createElements(giveaway.panel, (Settings.gv && ((main && this.esgst.giveawaysPath) || (source === 'gb' && Settings.gv_gb) || (source === 'ged' && Settings.gv_ged) || (source === 'ge' && Settings.gv_ge))) ? 'afterBegin' : 'beforeEnd', [{
          attributes: {
  ...
  gptw_addPoint(giveaway) {
    const entries = giveaway.entered || giveaway.ended || giveaway.created || !Settings.gptw_e ? giveaway.entries : giveaway.entries + 1;
    ...
    let color = null;
    for (const colors of Settings.gptw_colors) {
