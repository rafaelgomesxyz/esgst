...
class GiveawaysGiveawayCopyHighlighter extends Module {
  ...
  highlight(giveaways) {
    for (const giveaway of giveaways) {
      ...
      const { color, bgColor } = Settings.gch_colors.filter(colors => giveaway.copies >= parseInt(colors.lower) && giveaway.copies <= parseInt(colors.upper))[0] || { color: undefined, bgColor: undefined };
