...
class GiveawaysEnterLeaveGiveawayButton extends Module {
  ...
  async elgb_addButtons(giveaways, main, source) {
    giveaways.forEach(giveaway => {
      if (giveaway.sgTools || (main && (Settings.get('elgb_p') || this.esgst.createdPath || this.esgst.wonPath))) return;
  ...
  async elgb_addEntry(addButton, addingButton, errorButton, giveaway, removeButton) {
    ...
    try {
      ...
      if (responseJson.type === 'success') {
        ...
        if (Settings.get('elgb_b') && Settings.get('gb') && giveaway.gbButton && giveaway.gbButton.index === 1) {
  ...
  elgb_addButton(giveaway, main, source) {
    ...
    if (doAppend) {
      ...
      if (Settings.get('gv') && ((main && this.esgst.giveawaysPath) || (source === 'gb' && Settings.get('gv_gb')) || (source === 'ged' && Settings.get('gv_ged')) || (source === 'ge' && Settings.get('gv_ge')))) {
  ...
  async elgb_openPopup(giveaway, main, source, mainCallback) {
    ...
    if (giveaway.entered) {
      ...
      if (giveaway.started && !giveaway.ended && !giveaway.created && giveaway.level <= Session.counters.level.base && ((giveaway.id && ((games[giveaway.type][giveaway.id] && !games[giveaway.type][giveaway.id].owned && (!games[giveaway.type][giveaway.id].hidden || !Settings.get('hgebd'))) || !games[giveaway.type][giveaway.id])) || !giveaway.id)) {
        let set = new ButtonSet({
          color1: 'green',
          color2: 'grey',
          icon1: 'fa-plus-circle',
    ...
    if (description && Settings.get('elgb_f')) {
      let set = new ButtonSet({
        ...
        title2: 'Filtering...',
        callback1: async () => {
          await setSetting('elgb_filters', `${Settings.get('elgb_filters')}|${description.textContent.replace(/[^a-zA-Z]/g, '').toLowerCase()}`);
          set.remove();
        }
      }).set;
    ...
    if ((Settings.get('elgb_fp') && commentsContainer && commentsContainer.children.length) || description || (Settings.get('elgb_r') && (!Settings.get('elgb_r_d') || description)) || mainCallback) {
      if (mainCallback) {
  ...
  async elgb_enterGiveaway(giveaway, main, popup, source, callback) {
    ...
    if (responseJson.type === 'success') {
      ...
      if (main && Shared.esgst.gf && Shared.esgst.gf.filteredCount && Settings.get(`gf_enable${this.esgst.gf.type}`)) {
        this.esgst.modules.giveawaysGiveawayFilters.filters_filter(this.esgst.gf);
      }
      if ((!main || this.esgst.parameters.esgst) && this.esgst.gfPopup && this.esgst.gfPopup.filteredCount && Settings.get(`gf_enable${this.esgst.gfPopup.type}`)) {
  ...
  async elgb_leaveGiveaway(giveaway, main, source, callback) {
    ...
    if (responseJson.type === 'success') {
      ...
      if (!main && this.esgst.gfPopup && this.esgst.gfPopup.filteredCount && Settings.get(`gf_enable${this.esgst.gfPopup.type}`)) {
        this.esgst.modules.giveawaysGiveawayFilters.filters_filter(this.esgst.gfPopup);
      }
      if (callback) {
        callback();
