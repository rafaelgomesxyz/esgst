...
class GiveawaysGiveawayWinningChance extends Module {
  ...
  init() {
    if (Settings.gptw || (!this.esgst.enteredPath && (!this.esgst.wonPath || !Settings.cewgd || !Settings.cewgd_w || !Settings.cewgd_w_e))) return;
    this.esgst.endlessFeatures.push(this.gwc_addHeading.bind(this));
  }

  gwc_addChances(giveaways, main, source) {
    giveaways.forEach(giveaway => {
      if (giveaway.sgTools || (main && (this.esgst.createdPath || (this.esgst.wonPath && (!Settings.cewgd || !Settings.cewgd_w || !Settings.cewgd_w_e)) || this.esgst.newGiveawayPath || this.esgst.archivePath))) return;
      if (((giveaway.inviteOnly && ((main && (this.esgst.giveawayPath || this.esgst.enteredPath || (this.esgst.wonPath && Settings.cewgd && Settings.cewgd_w && Settings.cewgd_w_e))) || !main || giveaway.ended || giveaway.id)) || !giveaway.inviteOnly) && !giveaway.innerWrap.getElementsByClassName('esgst-gwc')[0]) {
        if (giveaway.started) {
          giveaway.gwcContext = createElements(giveaway.panel, (Settings.gv && ((main && this.esgst.giveawaysPath) || (source === 'gb' && Settings.gv_gb) || (source === 'ged' && Settings.gv_ged) || (source === 'ge' && Settings.gv_ge))) ? 'afterBegin' : 'beforeEnd', [{
            attributes: {
  ...
  gwc_addChance(giveaway) {
    ...
    entries = giveaway.entered || giveaway.ended || giveaway.created || !Settings.gwc_e ? giveaway.entries : giveaway.entries + 1;
    basicChance = entries > 0 ? giveaway.copies / entries * 100 : 100;
    basicChance = basicChance > 100 ? 100 : (basicChance <= 0 ? 0.01 : basicChance);
    if (Settings.gwc_a && !giveaway.ended && giveaway.startTime) {
    ...
    for (i = Settings.gwc_colors.length - 1; i > -1; --i) {
      colors = Settings.gwc_colors[i];
    ...
    if (Settings.gwc_h) {
      ...
      giveaway.headingName.style.color = Settings.gwc_a && !Settings.gwc_a_b ? advancedColor : basicColor;
      if (giveaway.image) {
        giveaway.image.classList.add('esgst-gwc-highlight');
        giveaway.image.style.color = `${Settings.gwc_a && !Settings.gwc_a_b ? advancedColor : basicColor}`;
        giveaway.image.style.boxShadow = `${Settings.gwc_a && !Settings.gwc_a_b ? advancedColor : basicColor} 0px 0px 0px var(--esgst-gwc-highlight-width, 3px) inset`;
      }
    }
    ...
    if (Settings.gwc_a && advancedChance) {
      if (Settings.gwc_a_b) {
        children.push({
    ...
    if ((this.esgst.enteredPath || this.esgst.wonPath) && Settings.gwr) {
      items.push({
        text: ' / ',
        type: 'node'
      });
  ...
  gwc_addHeading(context, main, source, endless) {
    if (this.esgst.createdPath || (this.esgst.wonPath && (!Settings.cewgd || !Settings.cewgd_w || !Settings.cewgd_w_e)) || !main) return;
