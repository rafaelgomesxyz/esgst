...
class CommentsCommentTracker extends Module {
  ...
  ct_addReadUntilHereButton(button, comment) {
    if (!button) {
      button = DOM.insert(comment.actions, 'beforeEnd', (
        <div
  ...
  ct_addCommentPanel(goToUnread, markRead, markUnread) {
    ...
    if (Settings.get('ct_a') && Shared.common.isCurrentPath('Messages')) {
      ...
      if (button) {
        if (this.esgst.sg) {
          newButton = DOM.insert(button, 'afterEnd', (
  ...
  ct_addDiscussionPanel(code, comments, container, context, count, diff, url, type, discussion, name) {
    const obj = {
      ...
      count,
      diff,
      panel: DOM.insert(context, this.esgst.giveawaysPath && !Settings.get('oadd') ? 'afterEnd' : 'beforeEnd', (
        <span>
          <span
